import '../database/block_history.dart';
import '../database/mempool.dart';
import '../database/robot_database.dart';
import '../database/tx_database.dart';
import 'block.dart';
import 'hash.dart';
...
/// Gets initilialized with [Blockchain.initBlockchain] method on server-side
...
class Blockchain {
  ...
  Future<Map<int, dynamic>?> validateBlock(Block block) async {
    ...
    if (topBlock.blockID != block.prevHash) {
      return {1: topBlock.blockID};
    }

    // step 2. checking for each transaction if it is absent in txDatabase
    // if any transaction is present in txDatabase, return error
    ...
    // if smth similar occurs, the FALSE is returned
    ...
    for (Map<bool, Map<String, int>> account in trState.values) {
      ...
      Set<String> allRobotIDsOfTransactions = Set<String>.from(
        List.from(receivedRobotsMap.keys) + List.from(sentRobotsMap.keys),
      );

      for (String robotID in allRobotIDsOfTransactions) {
  ...
  Future<void> printBlockchain() async {
    ...
    print("mempoolDatabase: ${await mempool.mempoolToString()}");
    print("--------------------------------------------------------");
  }
